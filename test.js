...
// Main Component
const DpoCard: React.FC<CardProps> = ({ data, dataFullWidth }) => {
  // Get the first report from data array
  let content = data?.[0] || {
    citation: [],
...
const HealthScoreContainer = styled(RoundedBox)`
  display: flex;
  flex-direction: column;
  align-items: center;
  padding: 1.5rem;
...
const HealthScoreHeader = styled(FieldLabel)`
  font-size: 1rem;
  font-weight: 600;
  color: #374151;
  margin-bottom: 1rem;
...
const HealthScore: React.FC<{ score: number }> = ({ score }) => {
    return (
        <HealthScoreContainer>
            <HealthScoreHeader>Overall Health Score</HealthScoreHeader>
        ...
        <ReportSidebar>
            ...
            <ReportsContainer>
                {availableReports.map((report: GazetteReport) => {
                    ...
                    // Create a clean title from reportType
...
const ReportDetailsView: React.FC<{
    ...
    return (
        <>
            <ContentHeader>
                <ContentTitle>{getReportTitle(reportData.reportType)}</ContentTitle>
                <ContentMeta>
                    <MetaItem>
